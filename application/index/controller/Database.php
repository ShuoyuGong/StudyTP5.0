...
class Database
{
  ...
  public function transaction()
  {
    ...
    // 书写事务
    try {
      // 删除user_info表中id等于20的数据
      $a = Db::table("user_info")->delete(10);
      // 删除不存在的数据
      $b = Db::table("user_info")->delete(60);
      // 判断数据是否删除成功
      if (!$b) {
        // 抛出异常 可通过getMassage接收
        throw new \Exception("删除id60的数据没有成功");
      }
      // 如果两个删除指令都执行成功之后 执行提交操作
      Db::commit();
      echo "删除命令执行完毕";
    } catch (\Exception $e) {
      // 回滚事务
